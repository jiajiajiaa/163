<template>
  <div class="phoneLogin">
    ...
    <div class="loginContainer">
      ...
      <div class="register" @click="checkShow(4)">
        <span>邮箱账号注册 ></span>
      </div>
    </div>
  </div>
  ...
  export default {
    ...
    methods:{
      ...
      yanzheng(){
        ...
        //验证初次输入密码的格式
        if(this.firstPwd & PwdReq.test(this.firstPwd)){
          this.isShowFirstPwd = false
        }else{
          this.isShowFirstPwd = true
        }
        //验证两次输入的密码是否一致
        if(this.nextPwd & this.firstPwd === this.nextPwd){
        ...
        if(!this.isShowTestPhone & !this.isShowYanzhengma & !this.isShowFirstPwd & !this.isShowNextPwd & !this.isShowRequiredTiaokuan){
          alert('验证成功，发送注册请求');
        }
      }
    }
